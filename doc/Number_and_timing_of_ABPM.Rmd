...
\newpage
Figure 1: Illustration of blood pressure sampling variations following a consecutive and distributed sampling strategy when blood pressure is measured every 30 minutes.
`r fp_par(line_spacing = 1)`
...
Black lines show predicted blood pressure from a linear regression model with a natural cubic spline to capture nonlinear patterns in blood pressure over time.
`r fp_par(line_spacing = 1)`
Shaded area around the black lines show 95% confidence limits for the blood pressure estimates.
`r fp_par(line_spacing = 1)`
The grey dashed line shows the mean systolic blood pressure during sleep for participants in the Coronary Artery Risk Development in Young Adults study (top row) and the Jackson Heart Study (bottom row)
`r fp_par(line_spacing = 1)`
...
Blood pressure was estimated using linear regression with a natural cubic spline to capture nonlinear patterns in blood pressure over time. 
`r fp_par(line_spacing = 1)`
Shaded ribbons around the black lines show 95% confidence limits for the blood pressure estimates.
`r fp_par(line_spacing = 1)`
The grey dashed line shows the mean diastolic blood pressure during sleep for participants in the Coronary Artery Risk Development in Young Adults study (top row) and the Jackson Heart Study (bottom row)
`r fp_par(line_spacing = 1)`
